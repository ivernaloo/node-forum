...
export default class Login extends React.Component {
  ...
  handleChange(name, e) {
    this.state[name] = e.target.value;  // 点击时的名字
  }
  ...
  handleSendCode(e) {
    ...
    requestResetPassword(this.state.email)
      .then(ret => {
        $btn.button('reset');
        this.setState({sent: true}); // 这里的对象应该是一个object
  ...
  handleReset(e) {
    ...
    resetPassword(this.state.code, this.state.email, this.state.password) // 设置新的代码
      .then(ret => {
        $btn.button('reset');
        alert('修改成功！');  
        redirectURL('/login');
    ...
    return (
      <div style={{width: 400, margin: 'auto'}}>
        <div className="panel panel-primary">
          <div className="panel-heading">重新设置密码</div>
